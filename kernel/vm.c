...
mappages_new(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  for(;;){
    if((pte = walk(pagetable, a, 1)) == 0)
      return -1;
    // just bypass the if-panic statement. 
    //if((*pte & PTE_V)){
    //  panic("mappages: remap");
    //}
    *pte = PA2PTE(pa) | perm | PTE_V;
...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < va + npages*PGSIZE; a += PGSIZE){
    ...
    if(do_free){
      ...
      //kfree((void*)pa); //original kfree statement here
      // make sure that physical address is not the zeroframe address. if it is true, premitted to kfree the physical address
